...
export type FilterStatus = 'All' | 'Watching' | 'Haven\'t Started' | 'Stopped' | 'Up to Date' | 'Favorites' | 'Rewatches';
...
export function useShows() {
  ...
  useEffect(() => {
    fetch('http://127.0.0.1:8000/api/shows?limit=5000')
      .then(res => res.json())
      .then(data => {
        setShows(data);
        setLoading(false);
  ...
  const filteredShows = useMemo(() => {
    ...
    if (filter !== 'All') {
      result = shows.filter(show => {
        if (filter === 'Stopped') return show.status === 'stopped';
        if (filter === 'Haven\'t Started') return show.status === 'not_started_yet' || show.status === 'watch_later';
        if (filter === 'Watching') return show.status === 'continuing';
        if (filter === 'Up to Date') return show.status === 'up_to_date';
        if (filter === 'Favorites') return show.is_favorite;
        if (filter === 'Rewatches') return (show.watched_count || 0) > 1;
        return true;
      });
    }
    ...
    return result;
  }, [shows, filter, sort, genreFilter, languageFilter, countryFilter]);
